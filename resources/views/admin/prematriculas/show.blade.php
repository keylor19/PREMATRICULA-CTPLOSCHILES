        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                <form method="POST" action="{{ route('admin.prematriculas.destroy', $prematricula) }}"
                    onsubmit="return confirm('¿Seguro que querés eliminar esta prematrícula? Se borrarán todos los datos y se liberará el cupo.')">
                    @csrf
                    @method('DELETE')
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white shadow-sm rounded-lg p-6 border border-gray-200">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div><dt class="text-gray-400">Cédula</dt><dd class="text-gray-900 font-medium">{{ $prematricula->tutor->cedula }}</dd></div>
                    <div><dt class="text-gray-400">Teléfono principal</dt><dd class="text-gray-900 font-medium">{{ $prematricula->tutor->telefono_principal }}</dd></div>
                    @if ($prematricula->tutor->telefono_secundario)
                        <div><dt class="text-gray-400">Teléfono secundario</dt><dd class="text-gray-900 font-medium">{{ $prematricula->tutor->telefono_secundario }}</dd></div>
                    @endif
                    <div><dt class="text-gray-400">Correo</dt><dd class="text-gray-900 font-medium">{{ $prematricula->tutor->email }}</dd></div>
                    @if ($prematricula->tutor->ocupacion)
                        <div><dt class="text-gray-400">Ocupación</dt><dd class="text-gray-900 font-medium">{{ $prematricula->tutor->ocupacion }}</dd></div>
                    @endif
                </dl>
            </div>
            ...
            {{-- Nivel y taller --}}
            <div class="bg-white shadow-sm rounded-lg p-6 border border-gray-200">
                <h3 class="text-base font-semibold text-gray-900 mb-4">Nivel y sección solicitada</h3>
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <ul class="space-y-2">
                    @foreach ($prematricula->documentos as $doc)
                        <li class="text-sm text-gray-600 flex items-center gap-2">
                            ...
                            {{ $doc->nombre_original }}
                            <span class="text-xs text-gray-400">({{ str_replace('_', ' ', $doc->tipo) }})</span>
                        </li>
                    @endforeach
                </ul>
            </div>
